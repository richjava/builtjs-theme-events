...
export default function Header1({ content }: any) {
  ...
  return (
    <header id="header-1" className="template relative">
      <Link className="hidden lg:block lg:absolute top-0 left-0 z-50 xl:ml-20 w-32 xl:w-40" href="/">
          <Image
            id="logo-anchor"
            ...
            <div className="hidden lg:flex lg:items-center">
              {primaryMenuItems &&
                primaryMenuItems.map((menuItem: any, i: number) => {
                  return (
                    <Link className={`primary-nav-link flex items-start border-r border-l ${
                      router.pathname === `/${menuItem.attributes.slug}`
                        ? "active"
                        : ""
                    }`} href={`/${menuItem.attributes.slug}`} key={i}>
                        <Image
          ...
          <div className="flex items-center">
            {secondaryMenuItems &&
              secondaryMenuItems.map((menuItem: any, i: number) => {
                return (
                  <Link className={`secondary-nav-link hidden lg:block  ${
                    router.pathname === `/${menuItem.attributes.slug}`
                      ? "active"
                      : ""
                  }`} href={`/${menuItem.attributes.slug}/`} key={i}>
            ...
            <Link className="hidden lg:block lg:ml-4 px-6 py-5 xl:px-10 xl:py-4 xl:py-6 text-white lg:text-sm lg:tracking-wider xl:tracking-widest hover:text-white bg-secondary hover:bg-secondary-dark uppercase focus:outline-none focus:text-white focus:bg-secondary transition duration-200 ease-in-out" href="/subscribe/">
                Join Us
            </Link>
            <Link className="block lg:hidden p-4 text-white tracking-widest hover:text-white bg-secondary hover:bg-secondary-dark uppercase focus:outline-none focus:text-white focus:bg-secondary transition duration-200 ease-in-out" href="/subscribe/">
